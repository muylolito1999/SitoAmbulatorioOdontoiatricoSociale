...
const roleStyles: { icon: ReactNode; borderColor: string; iconBg: string; bulletColor: string }[] = [
  {
    icon: (
  ...
  },
  {
    ...
    bulletColor: 'bg-secondary-500',
  },
];

export default function VolunteerPage() {
  const { content } = useContent();
  const { volunteerPage, contact } = content;
    ...
    <div className="pt-24">
      ...
      <section className="py-20 sm:py-28">
        <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal>
            <SectionHeading title={volunteerPage.title} />
          ...
          <div className="grid gap-8 md:grid-cols-2">
            {volunteerPage.roles.map((role, i) => {
              const style = roleStyles[i % roleStyles.length]!;
              return (
                <ScrollReveal key={role.title} delay={(i + 1) * 100}>
                  <Card className={`h-full border-t-4 ${style.borderColor}`}>
                    <div className={`mb-4 flex h-14 w-14 items-center justify-center rounded-xl ${style.iconBg}`}>
                      {style.icon}
                    </div>
                    <h3 className="text-xl font-bold text-neutral-900">{role.title}</h3>
                    <ul className="mt-4 space-y-2">
                      {role.items.map((item) => (
                        <li key={item} className="flex items-center gap-2 text-sm text-neutral-700">
                          <span className={`h-1.5 w-1.5 shrink-0 rounded-full ${style.bulletColor}`} />
                          {item}
                        </li>
                      ))}
                    </ul>
          ...
          <ScrollReveal>
            <div className="mx-auto max-w-2xl">
              <ul className="space-y-4">
                {volunteerPage.benefits.map((benefit) => (
